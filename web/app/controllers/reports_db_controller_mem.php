...
class ReportsDbController extends AppController
{
    ...
    var $name = "ReportsDb";
    var $uses = array('Cdrs', 'Cdr','CdrsRead','CdrRead','Reports','CdrMem');
    var $components = array('RequestHandler');
    var $helpers = array('javascript', 'html', 'Common','AppResource');
    ...
    public function summary_bak($type = 1)
    {
    ...
    public function usagereport($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrMem->get_usage_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $data = $this->CdrMem->query($sql);
                    $data = $this->_format_client_cdr_data($data,$ingress_clients,$egress_clients);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_usagereport($start_date, $report_max_time, $fields,$out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $cdr_report_data = $this->CdrsRead->query($sql1);
                    $sql2 = $this->CdrMem->get_usage_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $client_cdr_data = $this->CdrMem->query($sql2);
                    $client_cdr_data = $this->_format_client_cdr_data($client_cdr_data,$ingress_clients,$egress_clients);
    ...
    public function location()
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrsMem->get_location_from_client_cdr($report_max_time, $end_date,$egress_trunk_name,$ingress_trunk_name);
                    $data = $this->CdrsMem->query($sql);
                    $data = $this->_format_client_cdr_data($data,$ingress_clients,$egress_clients);
                }
                else
                {

                    $sql1 = $this->CdrsRead->get_location($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $table_name);
                    $cdr_report_data = $this->CdrsRead->query($sql1);
                    $sql2 = $this->CdrsMem->get_location_from_client_cdr($report_max_time, $end_date,$egress_trunk_name,$ingress_trunk_name);
                    $client_cdr_data = $this->CdrMem->query($sql2);
                    $client_cdr_data = $this->_format_client_cdr_data($client_cdr_data,$ingress_clients,$egress_clients);
    ...
    public function profit($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrMem->get_profit_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $data = $this->CdrMem->query($sql);
                    $data = $this->_format_client_cdr_data($data,$ingress_clients,$egress_clients);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_profit_cdrs($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $cdr_report_data = $this->CdrsRead->query($sql1);
                    $sql2 = $this->CdrMem->get_profit_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $client_cdr_data = $this->CdrMem->query($sql2);
                    $client_cdr_data = $this->_format_client_cdr_data($client_cdr_data,$ingress_clients,$egress_clients);
    ...
    public function qos_summary($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrMem->get_qos_summary_report_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $data = $this->CdrMem->query($sql);
                    $data = $this->_format_client_cdr_data($data,$ingress_clients,$egress_clients);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_qos_cdrs($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $cdr_report_data = $this->CdrsRead->query($sql1);
                    $sql2 = $this->CdrMem->get_qos_summary_report_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $client_cdr_data = $this->CdrMem->query($sql2);
                    $client_cdr_data = $this->_format_client_cdr_data($client_cdr_data,$ingress_clients,$egress_clients);
    ...
    public function summary($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrMem->get_summary_report_from_client_cdr($report_max_time, $end_date, $type,$egress_trunk_name,$ingress_trunk_name);
                    $data = $this->CdrMem->query($sql);
                    $data = $this->_format_client_cdr_data($data,$ingress_clients,$egress_clients);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_cdrs($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $cdr_report_data = $this->CdrsRead->query($sql1);
                    $sql2 = $this->CdrMem->get_summary_report_from_client_cdr($report_max_time, $end_date, $type);
                    $client_cdr_data = $this->CdrMem->query($sql2);
                    $client_cdr_data = $this->_format_client_cdr_data($client_cdr_data,$ingress_clients,$egress_clients);
        ...
        if($type == 1){
            $select_fields =
                array(
                    ...
                    23 => __('Local Cost', true),
                    24 => __('IJ Cost', true),

                    25 => __('Actual Rate / Avg Rate', true),
                    26 => __('Total Calls', true),
                    27 => __('Not Zero Calls', true),
                    28 => __('Success Calls', true),
                    29 => __('Busy Calls', true),
                );
            ...
            $select_fields =
                array(
                    ...
                    14 => __('Local Cost', true),
                    15 => __('IJ Cost', true),

                    16 => __('Actual Rate / Avg Rate', true),
                    17 => __('Total Calls', true),
                    18 => __('Not Zero Calls', true),
                    19 => __('Success Calls', true),
                    20 => __('Busy Calls', true),
                );
...
//        $this->set('show_header_arr',$show_header_arr);
    ...
    public function summary_report_client_arr(){
        ...
        $egress_clients = $this->Cdrs->get_egress_clients();
//        $this->set('ingress_clients', $ingress_clients);
//        $this->set('egress_clients', $ingress_clients);
...
//        $this->set('egress_clients', $egress_clients);
    ...
    public function _format_client_cdr_data($client_cdr_data,$ingress_clients,$egress_clients){
        ...
        $egress_arr = $this->CdrsRead->findAll_egress_id();
        $ingress_arr = $this->CdrsRead->findAll_ingress_id();
        $mem_cdr_table_name = Configure::read('memsql.cdr_table_name');

        foreach ($client_cdr_data as &$client_cdr_data_item){
            if(array_key_exists($mem_cdr_table_name,$client_cdr_data_item)){
                foreach ($client_cdr_data_item[$mem_cdr_table_name] as $client_cdr_field => $client_cdr_field_value){
                    if ($client_cdr_field == 'ingress_client_id'){
                        $client_cdr_field_value = isset($ingress_clients[$client_cdr_field_value]) ? $ingress_clients[$client_cdr_field_value] : '~';
                    }elseif ($client_cdr_field == 'egress_client_id'){
                        $client_cdr_field_value = isset($egress_clients[$client_cdr_field_value]) ? $egress_clients[$client_cdr_field_value] : '~';
                    }elseif ($client_cdr_field == 'egress_id'){
                        $client_cdr_field_value = isset($egress_arr[$client_cdr_field_value]) ? $egress_arr[$client_cdr_field_value] : '~';
                    }elseif ($client_cdr_field == 'ingress_id'){
                        $client_cdr_field_value = isset($ingress_arr[$client_cdr_field_value]) ? $ingress_arr[$client_cdr_field_value] : '~';
                    }
                    $client_cdr_data_item[0][$client_cdr_field] = $client_cdr_field_value;
                }
            }
        }
        return $client_cdr_data;
