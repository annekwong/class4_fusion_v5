    ...
    public function inout_report()
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrsRead->get_inout_from_client_cdr($report_max_time, $end_date);
                    $data = $this->CdrsRead->query($sql);
                }
                else
                {
    ...
    public function usagereport($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $data = $this->CdrsRead->get_usage_from_client_cdr($report_max_time, $end_date, $type);
                }
                else
                {
                    $data1 = $this->CdrsRead->get_usagereport($start_date, $report_max_time, $fields,$out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $data = $this->CdrsRead->get_usage_from_client_cdr($report_max_time, $end_date, $type);
//pr($data1);pr($data);pr($show_fields);exit;
                    foreach ($data1 as $data1_item)
                    {
                        foreach ($data as $data_key => $data_item)
                        {
                            $is_new_data = true;
                            $break = false;
                            foreach ($show_fields as $show_fields_item)
                            {

                                if (strcmp($data1_item[0][$show_fields_item], $data_item[0][$show_fields_item]))
                                {//分组信息不相等 跳出分组信息循环 继续和$data的下一个数据比较
                                    $break = true;
                                    break;
                                }
                            }
                            if ($break)
                                continue;
                            //$data1_item 与$data_item 分组信息相等 将数据合并
                            foreach ($data1_item[0] as $key2 => $item2)
                            {
                                if (!array_key_exists($key2, $replace_fields))
                                    $data[$data_key][0][$key2] += $item2;
                                else
                                    $data[$data_key][0][$key2] = $item2;
                            }
                            $is_new_data = false;
    ...
    public function location()
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrsRead->get_location_from_client_cdr($report_max_time, $end_date);
                    $data = $this->CdrsRead->query($sql);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_location($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $table_name);
                    $sql2 = $this->CdrsRead->get_location_from_client_cdr($report_max_time, $end_date);
                    $data = $this->CdrsRead->get_location_from_two($sql1, $sql2, $orders, $show_fields);
                }
            }
            else
            {
    ...
    public function profit($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrsRead->get_profit_from_client_cdr($report_max_time, $end_date, $type);
                    $data = $this->CdrsRead->query($sql);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_profit_cdrs($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $sql2 = $this->CdrsRead->get_profit_from_client_cdr($report_max_time, $end_date, $type);
                    $sql = $this->CdrsRead->get_profit_from_two($sql1, $sql2, $type, $orders, $show_fields);
                    $data = $this->CdrsRead->query($sql);
                }
            }
            else
    ...
    public function qos_summary($type = 1)
    {
        ...
        if (isset($_GET['show_type']) || $is_preload)
        {
            ...
            if ($select_time_end > $system_max_end)
            {
                if ($is_from_client_cdr)
                {
                    $sql = $this->CdrsRead->get_qos_summary_report_from_client_cdr($report_max_time, $end_date, $type);
                    $data = $this->CdrsRead->query($sql);
                }
                else
                {
                    $sql1 = $this->CdrsRead->get_qos_cdrs($start_date, $report_max_time, $fields, $out_fields, $groups, $orders, $wheres, $type, $table_name);
                    $sql2 = $this->CdrsRead->get_qos_summary_report_from_client_cdr($report_max_time, $end_date, $type);
                    $data = $this->CdrsRead->get_qos_cdrs_two($sql1, $sql2, $type, $orders, $show_fields);
                }
            }
            else
            {
