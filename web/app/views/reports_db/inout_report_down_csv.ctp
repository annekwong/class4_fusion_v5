<?php foreach($show_fields as $field): ?><?php echo $replace_fields[$field]; ?>,<?php endforeach; ?>Inbound,,,Outbound,,,Profit,,Total Duration(min),ASR,ACD(min),PDD(ms),NPR Count,NPR,Calls,,,,<?php echo "\n"; ?>
<?php for($i=0;$i<count($show_fields);$i++):?>,<?php endfor; ?>Billed Time,Cost(USA),Avg Rate(USA),Billed Time,Cost(USA),Avg Rate(USA),(USA),%,,,,,,,Total,Not Zero,Success,Busy Calls,<?php echo "\n"; ?>
<?php
$i = 0;
$arr = array();
