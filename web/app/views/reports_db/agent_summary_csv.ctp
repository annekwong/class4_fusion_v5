...
if (!empty ($show_fields)) {
    $replaced_show_fields = [];
    foreach ($show_fields as $field_key) {
        $replaced_show_fields[] = isset($replace_fields[$field_key]) ? $replace_fields[$field_key] : $field_key;
    }
    echo implode(',', $replaced_show_fields) . ',';
}
...
<?php for ($ik = 0; $ik < $i; $ik++): ?>
    ...
    foreach (array_keys($show_fields) as $key): ?>
        ...
        if (in_array($key, array('ingress_client_id', 'egress_client_id', 'ingress_id', 'egress_id'))) {
            $layout .= $arr[$key][$ik] ? '"' . trim($arr[$key][$ik]) . '",' : ',';
        } else {
            $layout .= '"' . trim($arr[$key][$ik]) . '",';
        } ?>
    <?php endforeach; ?>

    <?php
    $resultArray = array(
        number_format($arr['abr'][$ik], 2, '.', '') . '%',
        number_format($arr['asr'][$ik], 2, '.', '') . '%',
        number_format($arr['acd'][$ik], 2, '.', ''),
        number_format($arr['aloc'][$ik], 2, '.', ''),
        number_format($arr['pdd'][$ik], 2, '.', ''),
        number_format($arr['ner'][$ik], 2, '.', '') . '%',
    );
    ...
    $resultArray[] = number_format($arr['success_calls'][$ik], 0, '.', '');
    $resultArray[] = number_format($arr['busy_calls'][$ik], 0, '.', '');


    $layout .= implode(',', $resultArray);
    echo $layout;
...
//    if ($type == 1) {
    ...
    $total_profit = abs($sum_egress_cost - $sum_call_cost);
...
//rate_display_as
    ...
    $count_group_field = 0;
    foreach (array_keys($show_fields) as $key):
        if ($count_group_field > 0) $tmpRes[] = ' ';
        $count_group_field++;
    endforeach;
    $resultArray = array(
        'Total',
        number_format($total_abr, 2, '.', '') . '%',
        number_format($total_asr, 2, '.', '') . '%',
        number_format($total_acd, 2, '.', ''),
        number_format($total_aloc, 2, '.', ''),
        number_format($total_pdd, 2, '.', ''),
        number_format($total_ner, 2, '.', '') . '%',
    );
    ...
    $resultArray[] = number_format($total_duration / 60, 2);
    $resultArray[] = number_format($total_bill_time / 60, 2);
    ...
    $resultArray[] = number_format($total_success_calls, 2, '.', '');
