...
class AppDisDbHelper extends AppHelper {
...
function create_cdr_report_link($rate_type,$release_cause_from_protocol_stack,$real_release_cause,$disconnect,$group_link_arr,$pice){
    ...
    if(!empty($release_cause_from_protocol_stack)) {
        $link=<<<EOD
		<a href="{$web_root}cdrreports_db/summary_reports/disconnect/{$release_cause_from_protocol_stack}/{$rate_type}/{$real_release_cause}{$pice_para}">
		<strong>{$disconnect}</strong></a>
EOD;
    } else {
        $link=<<<EOD
		<a href="{$web_root}cdrreports_db/summary_reports">
		<strong>{$disconnect}</strong></a>
EOD;
    }
