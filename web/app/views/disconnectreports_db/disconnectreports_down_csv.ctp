...
echo '"'.__($group_by_field_arr[$i],true).'",';
}
...
for ($ii=0;$ii<$c;$ii++){$f=$group_by_field_arr[$ii];
$field=$client_org[$i][0][$f];$group_link_arr[$f]=$field;
 if($f == 'ingress_id' || $f == 'egress_id'){
    continue;
}
if(trim($field)==''){echo "Unknown,";}else{echo '"'.$field.'",';}}}?>"<?php  echo  $client_org[$i][0]['release_cause_from_protocol_stack']; ?>","<?php  echo  strtoupper($client_org[$i][0]['cause']);?>","<?php if($rate_type=='org'){echo $client_org[$i][0]['disconnect'];}else{echo $client_org[$i][0]['disconnect'];}?>","<?php echo number_format($client_org[$i][0]['my_count']/$client_org[$i][0]['all_count']*100,3) ?>%",<?php if($rate_type=='org'){?>"<?php  echo  $client_org[$i][0]['release_cause']; ?>",<?php }?><?php echo "\n"; ?><?php }?><?php }}?>