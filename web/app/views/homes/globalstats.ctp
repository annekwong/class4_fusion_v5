                    ...
                    <div>
                        ...
                        <select id="server_info" style="width:180px;">
                            <?php foreach ($limit_servers as $limit_server): ?>
                                <option value="<?php echo $limit_server[0]['lan_ip'] . ':' . $limit_server[0]['lan_port'] ?>"><?php echo $limit_server[0]['name'] ?></option>
                            <?php endforeach; ?>
        ...
        <div style="width: 100%;; margin: 0px ">
            <fieldset>
                ...
                <table class="list list-form footable table table-striped tableTools table-bordered  table-white table-primary">
                    ...
                    <tbody  id='syslimit'>
                        <tr class="row-1">
                            <td><?php echo __('Channel', true); ?></td>
...
<script type="text/javascript">
    function set_sys_limit(data,server_info) {
        if(data != 'null') {
            data = eval('(' + data + ')');
            ...
            if (data.status == 0){
//                var msg = '<?php //__('Connection to Server at [%s] is not available.'); ?>//'.replace('%s',server_info);
...
//                    if (data.status == 0){
//                        jGrowl_to_notyfy('<?php //__('This switch server is not connected'); ?>//', {theme: 'jmsg-error'});
//                    }
                    set_sys_limit(data,server_info);
                }
            });
        });
