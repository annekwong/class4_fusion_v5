...
//循环的表格
    ...
    <td class='in-decimal'>".$app->show_release_cause($client_org[$i][0]['release_cause']) ." </td>
</tr>";
            if($i>0){
//取最后1个分组条件
                $field_name=$group_by_field_arr[$c-1];
        ...
        <table class="list footable table table-striped dynamicTable tableTools table-bordered  table-white table-primary"  style="width: 100%">
            ...
            <tr>
                ...
                if(!empty($group_by_field_arr)){
                    ...
                    for ($i=0;$i<$c;$i++){
                        if($group_by_field_arr[$i]=='ingress_id' || $group_by_field_arr[$i]=='egress_id'){
                            continue;
                        }
                        echo " <th rel='8'  width='6%'>&nbsp;&nbsp;&nbsp;&nbsp; ".$appCommon->show_order($group_by_field_arr[$i],__($group_by_field_arr[$i],true))."  &nbsp;&nbsp;</th>";
                    }

                }?>
                <th width="10%" rel="0" >&nbsp;<?php echo $appCommon->show_order('release_cause_from_protocol_stack','Release Cause')?>&nbsp;</th>
                <th width="10%" rel="0" >&nbsp;<?php echo $appCommon->show_order('disconnect',__('Counts',true))?> &nbsp;</th>
                <th width="10%" rel="2" class="cset-1"><?php echo $appCommon->show_order('my_count',__('Counts %',true))?> </th>
                <!--<th width="30%" rel="2" class="cset-1">&nbsp; &nbsp;</th>-->

                <?php if($rate_type=='org'){?>
                    <th width="30%"  class='last' rel="2" class="cset-1">&nbsp;<?php echo $appCommon->show_order('release_cause',__('SIP Response',true))?> &nbsp;</th>
                <?php }?>
            </tr>
            </thead>
            <tbody >
            ...
            for ($i=0;$i<$size;$i++){
                if(!isset($all_count))
                {
                    $all_count =  $client_org[$i][0]['all_count'];
                }
                if ($client_org[$i][0]['release_cause_from_protocol_stack'] == '0'){
                    continue;
                }
...
//        }
                ...
                <tr class=" row-<?php echo $i%2+1?>"   style="color: #4B9100">
                    ...
                    <td class="in-decimal">
                        <?php  echo  $client_org[$i][0]['release_cause_from_protocol_stack'] ? $client_org[$i][0]['release_cause_from_protocol_stack'].':' : ''; ?>
                        <?php
                        if ($client_org[$i][0]['cause']){
                            echo strtoupper($client_org[$i][0]['cause']);
                        }elseif($client_org[$i][0]['release_cause_from_protocol_stack']){
                            $appCommon->get_response($client_org[$i][0]['release_cause_from_protocol_stack']);
                        }elseif ($client_org[$i][0]['release_cause']){
                            if (is_array($client_org[$i][0]['release_cause'])) {
                                $arr = array();
                                foreach ($client_org[$i][0]['release_cause'] as $item) {
                                    $arr[] = $appCommon->show_release_cause($item);
                                }
                                echo implode(", ", $arr);
                            } else {
                                echo $appCommon->show_release_cause($client_org[$i][0]['release_cause']);
                            }
                        }else{
                            echo "--";
                        } ?>
                    ...
                    <td class="in-decimal">
                        ...
                        foreach($group_link_arr as $key=>$value){
                            if($key == 'egress_alias'){
                                $group_link_arr[$key] = isset($trunksMap[$field]) ? $trunksMap[$field] : 0;
                            }
                            if($key == 'ingress_alias'){
                                $group_link_arr[$key] = isset($trunksMap[$field]) ? $trunksMap[$field] : 0;
                            }
                        }

                        if($rate_type=='org'){
                            echo $appDisDb->create_cdr_report_link($rate_type,$client_org[$i][0]['release_cause_from_protocol_stack'],$client_org[$i][0]['real_release_cause'],number_format($client_org[$i][0]['disconnect'], 0),$group_link_arr,explode("?",$appCommon->get_curr_url()));
                        }
                        else
                        {
                    ...
                    <td class="in-decimal"><?php echo number_format($client_org[$i][0]['my_count']/$all_count*100,3) ?>%</td>
                    <!--<td class="in-decimal">
                <div class="bar"><div style="font-size:1.2em;width: <?php echo number_format($client_org[$i][0]['my_count']/$all_count*100,3)?>% ;"><?php echo number_format($client_org[$i][0]['my_count']/$all_count*100,3) ?>%&nbsp;</div></div>

            </td>-->
                    <?php if($rate_type=='org'){?>
                        <td class="in-decimal"><?php  echo  $app->show_release_cause($client_org[$i][0]['release_cause']); ?> </td>
                    <?php }?>
                </tr>
            <?php }?>
            </tbody>
