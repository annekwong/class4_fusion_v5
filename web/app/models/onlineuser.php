...
//$page = $page->checkRange($page);//检查当前页范围
    ...
    public function get_network_total_single($date, $server)
    {
        ...
        $sql = "SELECT report_time, max(call) as call, max(ingress_cps) as cps, max(channels) as channel  FROM qos_total WHERE  report_time = '{$date}' {$server_condition} GROUP BY report_time  ORDER BY report_time ASC";
        return $this->query($sql);
    }

    public function get_time_by_duration($duration)
    {
    ...
    public function get_network_total($duration, $server)
    {
        ...
        $sql = "SELECT to_char(report_time,'YYYY-MM-DD HH24:MI:00')  as report_time, max(call) as call, max(cps) as cps, max(channels) as channel FROM qos_total WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}'
AND CURRENT_TIMESTAMP $server_condition GROUP BY to_char(report_time,'YYYY-MM-DD HH24:MI:00') ORDER BY report_time ASC";
        if ($duration >= 6) {
            $sql = "SELECT report_time::date as report_time, max(call) as call, max(cps) as cps, max(channels) as channel FROM qos_total WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}'
AND CURRENT_TIMESTAMP $server_condition GROUP BY report_time::date ORDER BY report_time ASC";
        }
...
// end channel
    ...
    public function get_draw_trunk_data_client($type, $duration, $trunk, $trunk_ip, $server, $zero_data = array())
    {
        ...
        if (is_array($trunk)) {
            $ids = implode(',', $trunk);
            $sql = "SELECT resource_id as res_id, alias as name FROM resource WHERE resource_id in ({$ids})";
...
ORDER BY report_time ASC";
            if ($duration >= 7) {
                $sql_item = "SELECT to_char(report_time, 'YYYY-MM-DD HH24:00:00') as report_time, max(call) as call FROM (SELECT
...
ORDER BY 1 ASC";
            ...
            foreach ($item_results as $item_item) {
                ...
                if (is_array($trunk)) {
                    $key = $this->getArrayKeyByObjectValue($draw_data['call'][0]['data'], $time_key);
                    if ($key) {
                        $draw_data['call'][0]['data'][$key][1] += intval($item_item[0]['call']);
                    }
                }
            }
...
//        }
        ...
        $fields = "sum(ingress_cps) as cps, sum(ingress_channels) as channel";

        if ($type == 1) {
            $fields = "sum(egress_cps) as cps, sum(egress_channels) as channel";
        }

        $resIdSql = <<<SQL
    SELECT res_id, max(call) as call
    from qos_resource
    WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition
    GROUP BY res_id
    ORDER BY call desc {$limit}
SQL;

        $resourceIds = $this->query($resIdSql);
        $resourceIdsArray = array();

        foreach ($resourceIds as $resourceId) {
            array_push($resourceIdsArray, $resourceId[0]['res_id']);
        }
        $resourceIdsString = implode(',', $resourceIdsArray);

        $resourceIdsCondition = "";
        if (count($resourceIdsArray) > 0) {
            $resourceIdsCondition = " AND res_id IN ({$resourceIdsString})";
        }

        if ($duration >= 6) {

            $sql_total = <<<SQL
SELECT report_time, 0 as call, 0 as cps, 0 as channel 
FROM (
SELECT report_time::date as report_time, res_id, max(call) as call, max(cps) as cps, max(channels) as channel
FROM (
SELECT report_time, res_id, call, cps, channels
FROM qos_resource
WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition $resourceIdsCondition
ORDER BY report_time ASC
) as t2
GROUP BY report_time::date, res_id
ORDER BY report_time ASC
) as t1
GROUP BY t1.report_time
ORDER BY report_time ASC;
SQL;

//            $sql_total = <<<SQL
//   SELECT
//report_time, sum(call) as call, sum(cps) as cps, sum(channel) as channel
//FROM (SELECT res_id, report_time::date as report_time, max(call) as call, max(cps) as cps, max(channels) as channel
//FROM qos_resource
//WHERE
//report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition $resourceIdsCondition
//GROUP BY report_time::date, res_id
//ORDER BY report_time ASC) as t1
//GROUP BY t1.report_time;
//SQL;


//            $sql_total = <<<SQL
//    SELECT report_time, call, cps, channel
//    FROM (
//        SELECT report_time::date as report_time, sum(call) as call, {$fields}
//        FROM qos_total
//        WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP GROUP BY report_time::date
//    ) AS t
//    ORDER BY 1 asc ;
//SQL;
        } else {
...
SELECT report_time, 0 as call, 0 as cps, 0 as channel 
FROM (
SELECT to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, res_id, max(call) as call, max(cps) as cps, max(channels) as channel
FROM (
SELECT report_time, res_id, call, cps, channels
FROM qos_resource
WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition $resourceIdsCondition
...
GROUP BY to_char(report_time,'YYYY-MM-DD HH24:MI:00'), res_id
ORDER BY report_time ASC
) as t1
GROUP BY t1.report_time
ORDER BY report_time ASC;
...
//   SELECT
//report_time, sum(call) as call, sum(cps) as cps, sum(channel) as channel
//FROM (SELECT to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, max(call) as call, max(cps) as cps, max(channels) as channel
//FROM (
//SELECT report_time, res_id, call, cps, channels
//FROM qos_resource
//WHERE
...
//    SELECT report_time, call, cps, channel
//    FROM (
//        SELECT to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, sum(call) as call, {$fields}
//        FROM qos_total
//        WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP GROUP BY to_char(report_time,'YYYY-MM-DD HH24:MI:00')
//    ) AS t
//    ORDER BY 1 asc ;
//SQL;
        }
...
//                $channel_data = array();
                ...
                if ($duration >= 6) {
                    $sql_item = "SELECT
report_time::date as report_time, max(call) as call, max(cps) as cps, max(channels) as channel
FROM qos_ip
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
...
ORDER BY report_time ASC";
                    ...
                    $sql_item = "SELECT
to_char(report_time,'YYYY-MM-DD HH24:MI:00'), max(call) as call, max(cps) as cps, max(channels) as channel
FROM qos_ip
WHERE 
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND ip_id = {$item[0]['resource_ip_id']} AND direction = {$type} $server_condition
...
ORDER BY report_time ASC";
                ...
                foreach ($item_results as $item_item) {
                    ...
                    if (array_key_exists($time_key, $ip_call_zero_data)) {
                        ...
                        $key = $this->getArrayKeyByObjectValue($draw_data['cps'][0]['data'], $time_key);
                        if ($key) {
                            $draw_data['cps'][0]['data'][$key][1] += intval($item_item[0]['cps']);
                        }

                        $key = $this->getArrayKeyByObjectValue($draw_data['channel'][0]['data'], $time_key);
                        if ($key) {
                            $draw_data['channel'][0]['data'][$key][1] += intval($item_item[0]['channel']);
                        }
                    }
...
//                    array_push($channel_data, array(strtotime($item_item[0]['report_time']) * 1000, (float) $item_item[0]['channel']));
            ...
            if (!ctype_digit($trunk)) {
                ...
                foreach ($result as $item) {
                    ...
                    $sql_item = "SELECT
report_time::date as report_time, max(call) as call
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
ORDER BY report_time ASC";
                        ...
                        $sql_item = "SELECT
to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, max(call) as call
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
//                    $cps_data = array();
                    ...
                    if ($duration >= 6) {
                        $sql_item = "SELECT
report_time::date as report_time, max(cps) as cps
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
ORDER BY report_time ASC";
                        ...
                        $sql_item = "SELECT
to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, max(cps) as cps
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
//                    $channel_data = array();
                    ...
                    if ($duration >= 6) {
                        $sql_item = "SELECT
report_time::date as report_time, max(channels) as channel
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
ORDER BY report_time ASC";
                        ...
                        $sql_item = "SELECT
to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, max(channels) as channel
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
ORDER BY report_time ASC";
                        ...
                        $sql_item = "SELECT
to_char(report_time,'YYYY-MM-DD HH24:MI:00') as report_time, max(call) as call, max(cps) as cps, max(channels) as channel
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
ORDER BY report_time ASC";
                    ...
                    foreach ($item_results as $item_item) {
                        ...
                        if (array_key_exists($time_key, $item_call_zero_data)) {
                            ...
                            $key = $this->getArrayKeyByObjectValue($draw_data['call'][0]['data'], $time_key);
                            $key1 = $this->getArrayKeyByObjectValue($draw_data['cps'][0]['data'], $time_key);
                            $key2 = $this->getArrayKeyByObjectValue($draw_data['channel'][0]['data'], $time_key);
                            if ($key) {
                                $draw_data['call'][0]['data'][$key][1] += intval($item_item[0]['call']);
                            }
                            if ($key1) {
...
//                        array_push($channel_data, array(strtotime($item_item[0]['report_time']) * 1000, (float) $item_item[0]['channel']));
    ...
    private function getArrayKeyByObjectValue($array, $objectValue) {
        $returnValue = false;

        foreach ($array as $key => $item) {
            if (isset($item[0]) && $item[0] == $objectValue) {
                $returnValue = $key;
                break;
            }
        }
