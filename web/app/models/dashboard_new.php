...
//处理 第一部分：  ajax_text1
    public function ajax_text1($time){
        ...
        $sql = $this->ajax_text1_get_qos($start_time,$end_time);
...
//        $max_time = date('Y-m-d H:i:sO',strtotime($max_time)+$interval*$i);
        ...
        $sql = "select $field from qos_route_report $where $group order by report_time";
...
//        //如果数据点没有数据，置空
    ...
    public function ajax_chart1_get_qos($time,$start_time,$end_time,$tab_value){
        ...
        if($time != 1){
            $group = " report_time ";
            $field = " to_char(report_time, 'YYYY-MM-DD HH24:00:00') as stat_time ";
        }
        ...
        $sql = "select $field from qos_route_report $where $group ";
    ...
    public function ajax_chart1_get_report($time,$start_time, $end_time, $tab_value){
        ...
        if($time != 1){
            $group = " report_time ";
            $out_field = " to_char(report_time, 'YYYY-MM-DD HH24:00:00') as stat_time ";
        }
...
//处理 第三部分：  ajax_text2
    public function ajax_text2($time){
        $tz = $this->get_sys_timezone();
        //1,最近一小时
        if($time==1){
        ...
        $bill_time = $data['bill_time'] + 0;
        $not_zero_calls = $data['not_zero_calls'] + 0;
        $busy_calls = $data['busy_calls'] + 0;
        $cancel_calls = $data['cancel_calls'] + 0;
    ...
    public function ajax_text2_get_qos($start_time,$end_time){
        ...
        $sql = "select $field from qos_route_report $where ";
...
//第四部分：  ajax_table1
    //$time 1->1小时， 24->24小时
    public function ajax_table1($time, $sort, $limit){
        $tz = $this->get_sys_timezone();
        $end_time = $this->sys_time;
        ...
        $sql = $this->ajax_table1_get_report($start_time,$end_time) . $sort . $limit;
...
//        var_dump($sql);exit;
        ...
        foreach($rst as $k => $v){
            ...
            $ingress_busy_calls = $v[0]['ingress_busy_calls'] + 0;
            $ingress_cancel_calls = $v[0]['ingress_cancel_calls'] + 0;



            $data[$k]['acd'] = !empty($not_zero_calls) ? round($ingress_bill_time / $not_zero_calls / 60, 2) : 0;

            $asr_ = $ingress_busy_calls + $ingress_cancel_calls + $not_zero_calls;
    ...
    public function count_ajax_table1($time){
        $tz = $this->get_sys_timezone();
        $end_time = $this->sys_time;
        $end_time = strtotime(date('Y-m-d H:00:00',$end_time));
        ...
        $sql = $this->ajax_table1_get_report($start_time,$end_time, true);
    ...
    public function ajax_table1_get_report($start_time, $end_time, $is_count = false){
...
//第五部分：  ajax_table2
    ...
    public function ajax_table2($time, $trunk, $sort){
        ...
        if($time==1){
            ...
            $start_time = mktime(0, 0 , 0,date("m"),date("d")-date("w")+1-3*11,date("Y"));
        } else {
    ...
    public function ajax_table2_get_qos($time,$start_time,$end_time,$trunk){
        ...
        if($time == 1){
            ...
            $group = " report_time ";
            $field = " to_char(report_time,'YYYY-MM-DD 00:00:00') as stat_time, ";
        } elseif($time == 3){
            $group = " report_time ";
            $field = " extract(WEEK FROM report_time) as stat_time, ";
        } else {
            $group = " report_time ";
            $field = " to_char(report_time, 'YYYY-MM-01 00:00:00') as stat_time, ";
        }
    ...
    public function ajax_table2_get_report($time, $start_time, $end_time,$trunk){
        if($time == 1){
            ...
            $group = " report_time ";
            $out_field = " to_char(report_time,'YYYY-MM-DD 00:00:00') as stat_time, ";
        } elseif($time == 3){
            $group = " report_time ";
            $out_field = " extract(WEEK FROM report_time) as stat_time, ";
        } else {
            $group = " report_time ";
            $out_field = " to_char(report_time, 'YYYY-MM-01 00:00:00') as stat_time, ";
        }
...
//第六部分：  ajax_chart2
    public function ajax_chart2($time, $which_value){
        ...
        switch($time){
            case 1:
                $start_time = strtotime( date('Y-m-d 00:00:00', strtotime('-29 day')) );
                break;
            case 2:
                $start_time = mktime(0, 0 , 0,date("m"),date("d")-date("w")+1-7*11,date("Y"));
                break;
            case 3:
                $start_time = strtotime( date('Y-m-01 00:00:00', strtotime('-11 month')) );
                break;
        ...
        if($time == 1){
            ...
            $data_time_arr = range( strtotime(date('Y-m-d 00:00:00',$start_time))*1000, strtotime(date('Y-m-d 00:00:00', $end_time))*1000, 24*3600000);
        } elseif($time == 2){
            $data_time_arr = array();
            for($i=11; $i>=0; $i--){

                $data_time_arr[] = mktime(0, 0 , 0,date("m"),date("d")-date("w")+1-7*$i,date("Y"))*1000;
            }
        } else {
            $data_time_arr = array();
            for($i=12; $i>=1; $i--){
                $data_time_arr[] = mktime(0, 0 , 0,date("m")-$i+1,1,date("Y"))*1000;
            }
        }
        ...
        //今天的数据从qos取，以前的从report取
        $sql1 = $this->ajax_chart2_get_qos($time, $qos_start_time, $end_time, $which_value);
        $sql2 = $this->ajax_chart2_get_report($time, $start_time, $report_end_time, $which_value);
    ...
    public function ajax_chart2_get_report($time,$start_time, $end_time, $which_value){
        ...
        switch($which_value){
            case 'minutes':
                ...
                $out_field .= " ,sum(ingress_bill_time) / 60 as minutes";
                break;
            case 'cost':
                $in_field .= " ,ingress_call_cost ";
                $out_field .= " ,sum(ingress_call_cost) as cost ";
                break;
        }
        ...
        {
            $where = <<<WHERE
report_time between '$start_time' and '$end_time' and EXISTS (SELECT 1 FROM agent_clients where client_id = ingress_client_id)
WHERE;
        }
        else
        {
...
SQL;
            ...
            foreach ($result as $item)
            {
                ...
                $sql_item = <<<SQLITEM
SELECT to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time,SUM($field) AS $field
...
SQLITEM;
                ...
                foreach ($item_results as $item_item)
                    array_push($call_data, array(strtotime($item_item[0]['stat_time']) * 1000, intval($item_item[0][$field])));
