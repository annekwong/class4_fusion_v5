...
//        $sql = "SELECT to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as report_time, sum(call) as call, sum(ingress_cps) as cps, sum(channels) as channel FROM qos_total WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}'
//AND CURRENT_TIMESTAMP $server_condition GROUP BY to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') ORDER BY report_time ASC";
        $sql = "SELECT to_char(report_time,'YYYY-MM-DD HH24:MI:00') as stat_time, sum(call) as call, sum(ingress_cps) as cps, sum(channels) as channel FROM qos_total WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}'
AND CURRENT_TIMESTAMP $server_condition GROUP BY report_time ORDER BY report_time ASC";
        if ($duration >= 7)
        {
            $sql = "SELECT to_char(report_time, 'YYYY-MM-DD HH24:00:00') as stat_time, max(call) as call, max(cps) as cps, max(channel) as channel FROM (SELECT report_time, sum(call) as call, sum(ingress_cps) as cps, sum(channels) as channel FROM qos_total WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}'
AND CURRENT_TIMESTAMP $server_condition GROUP BY report_time) AS t GROUP BY report_time
ORDER BY 1 ASC";
        }
        return $this->query($sql);
    }
    ...
    public function get_network_total_report_single($date, $server)
    {
        if ($server != NULL)
        {
            $server_condition = " AND server_ip = '{$server[0]}'";
        }
        else
        {
            $server_condition = '';
    ...
    public function get_network_total_report($duration, $server)
    {
        ...
        $sql = "SELECT to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(not_zero_calls) as not_zero_calls, sum(total_calls) as total_calls, sum(cancel_calls) as cancel_calls,sum(busy_calls) as busy_calls"
...
//            $pdd = $item[0]['pdd'];
    ...
    public function get_draw_trunk_data_report($direction, $duration, $server, $trunk, $show_type)
    {
        $time = $this->get_time_by_duration($duration);
        $server_condition = "";
        if ($server != NULL)
        ...
        foreach ($data as $key => $item)
        {
            ...
            $ready_data[$key]['acd'] = !empty($not_zero_calls) ? round($bill_time / $not_zero_calls / 60, 2) : 0;
            $ready_data[$key]['abr'] = !empty($total_calls) ? round($not_zero_calls / $total_calls * 100, 2) : 0;
            $asr_ = intval($busy_calls) + intval($cancel_calls) + intval($not_zero_calls);
            $ready_data[$key]['asr'] = !empty($asr_) ? round($not_zero_calls / $asr_ * 100, 2) : 0;
            $ready_data[$key]['pdd'] = !empty($not_zero_calls) ? round($pdd / $not_zero_calls) : 0;
            $ready_data[$key]['revenue'] = $ingress_client_cost_total;
            $ready_data[$key]['profitability'] = !empty($ingress_client_cost_total) ? round($revenue / $ingress_client_cost_total, 2)*100 : 0;
        }
        ...
        if($show_type == 'qos'){
            ...
            foreach ($ready_data as $item) {
                $time_key = $item['stat_time'];
                if (array_key_exists($time_key,$draw_data[$show_acd][0]['data'])){
                    $draw_data[$show_acd][0]['data'][$time_key] = array($time_key, $draw_data[$show_acd][0]['data'][$time_key][1] + (float)$item[$show_acd]);
                    $draw_data[$show_abr][0]['data'][$time_key] = array($time_key, $draw_data[$show_abr][0]['data'][$time_key][1] + (float)$item[$show_abr]);
                    $draw_data[$show_asr][0]['data'][$time_key] = array($time_key, $draw_data[$show_asr][0]['data'][$time_key][1] + (float)$item[$show_asr]);
                    $draw_data[$show_pdd][0]['data'][$time_key] = array($time_key, $draw_data[$show_pdd][0]['data'][$time_key][1] + (float)$item[$show_pdd]);
                }else {
                    $draw_data[$show_acd][0]['data'][$time_key] = array($time_key, (float)$item[$show_acd]);
                    $draw_data[$show_abr][0]['data'][$time_key] = array($time_key, (float)$item[$show_abr]);
                    $draw_data[$show_asr][0]['data'][$time_key] = array($time_key, (float)$item[$show_asr]);
                    $draw_data[$show_pdd][0]['data'][$time_key] = array($time_key, (float)$item[$show_pdd]);
                }
            }
            sort($draw_data[$show_acd][0]['data']);
            sort($draw_data[$show_abr][0]['data']);
            sort($draw_data[$show_asr][0]['data']);
            sort($draw_data[$show_pdd][0]['data']);
...
//            }
            ...
            foreach ($ready_data as $item) {
                ...
                $time_key = $item['stat_time'];
                if (array_key_exists($time_key,$draw_data[$show_acd][0]['data'])){
                    $draw_data[$show_revenue][0]['data'][$time_key] = array($time_key, $draw_data[$show_revenue][0]['data'][$time_key][1] + (float)$item[$show_revenue]);
                    $draw_data[$show_profitability][0]['data'][$time_key] = array($time_key, $draw_data[$show_profitability][0]['data'][$time_key][1] + (float)$item[$show_profitability]);
                }else {
                    $draw_data[$show_revenue][0]['data'][$time_key] = array($time_key, (float)$item[$show_revenue]);
                    $draw_data[$show_profitability][0]['data'][$time_key] = array($time_key, (float)$item[$show_profitability]);
                }
            }

            sort($draw_data[$show_revenue][0]['data']);
            sort($draw_data[$show_profitability][0]['data']);
        }
...
WHERE report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND resource_id is not null $server_condition  GROUP BY resource_id ORDER BY sum(total_calls) DESC {$limit}";
            ...
            foreach ($result as $item)
            {
                ...
                $call_data2 = array();
                $sql_item = "SELECT to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(not_zero_calls) as not_zero_calls, sum(total_calls) as total_calls, sum(cancel_calls) as cancel_calls,sum(busy_calls) as busy_calls"
...
//            $egress_cost_total = $item_results_item[0]['egress_cost_total'];
                    ...
                    $ready_data2[$key]['acd'] = !empty($not_zero_calls) ? round($bill_time / $not_zero_calls / 60, 2) : 0;
                    $ready_data2[$key]['abr'] = !empty($total_calls) ? round($not_zero_calls / $total_calls * 100, 2) : 0;
                    $asr_ = intval($busy_calls) + intval($cancel_calls) + intval($not_zero_calls);
                    $ready_data2[$key]['asr'] = !empty($asr_) ? round($not_zero_calls / $asr_ * 100, 2) : 0;
...
//                $test_arr = array();
                ...
                if($show_type == 'qos') {
                    ...
                    foreach ($ready_data2 as $item_results_item) {
                        $time_key = $item['stat_time'];
                        if (array_key_exists($time_key,$call_data2[$show_acd])){
                            $call_data2[$show_acd][$time_key] = array($time_key, $call_data2[$show_acd][$time_key][1] + (float)$item_results_item[$show_acd]);
                            $call_data2[$show_abr][$time_key] = array($time_key, $call_data2[$show_abr][$time_key][1] + (float)$item_results_item[$show_abr]);
                            $call_data2[$show_asr][$time_key] = array($time_key, $call_data2[$show_asr][$time_key][1] + (float)$item_results_item[$show_asr]);
                            $call_data2[$show_pdd][$time_key] = array($time_key, $call_data2[$show_pdd][$time_key][1] + (float)$item_results_item[$show_pdd]);
                        }else {
                            $call_data2[$show_acd][$time_key] = array($time_key, (float)$item_results_item[$show_acd]);
                            $call_data2[$show_abr][$time_key] = array($time_key, (float)$item_results_item[$show_abr]);
                            $call_data2[$show_asr][$time_key] = array($time_key, (float)$item_results_item[$show_asr]);
                            $call_data2[$show_pdd][$time_key] = array($time_key, (float)$item_results_item[$show_pdd]);
                        }
                    }
                    sort($call_data2[$show_acd]);
                    sort($call_data2[$show_abr]);
                    sort($call_data2[$show_asr]);
                    sort($call_data2[$show_pdd]);


//                    foreach ($ready_data2 as $item_results_item) {
//                        $report_time_key = $item_results_item['stat_time'];
...
//                    }
                    ...
                    foreach ($ready_data2 as $item_results_item) {
                        $time_key = $item['stat_time'];
                        if (array_key_exists($time_key,$call_data2[$show_acd])){
                            $call_data2[$show_revenue][$time_key] = array($time_key, $call_data2[$show_revenue][$time_key][1] + (float)$item_results_item[$show_revenue]);
                            $call_data2[$show_profitability][$time_key] = array($time_key, $call_data2[$show_profitability][$time_key][1] + (float)$item_results_item[$show_profitability]);
                        }else {
                            $call_data2[$show_revenue][$time_key] = array($time_key, (float)$item_results_item[$show_revenue]);
                            $call_data2[$show_profitability][$time_key] = array($time_key, (float)$item_results_item[$show_profitability]);
                        }
                    }
                    sort($call_data2[$show_revenue]);
                    sort($call_data2[$show_profitability]);


                    array_push($draw_data[$show_revenue], array(
                        'name' => $item[0]['name'],
                        'data' => $call_data2[$show_revenue],
                        'tooltip' => array(
                            'valueDecimals' => 2,
                        )
                    ));
                    array_push($draw_data[$show_profitability], array(
                        'name' => $item[0]['name'],
                        'data' => $call_data2[$show_profitability],
                        'tooltip' => array(
                            'valueDecimals' => 2,
                        )
                    ));
...
//            $egress_cost_total = $item_results_item[0]['egress_cost_total'];
                    ...
                    $ready_data3[$key]['acd'] = !empty($not_zero_calls) ? round($bill_time / $not_zero_calls / 60, 2) : 0;
                    $ready_data3[$key]['abr'] = !empty($total_calls) ? round($not_zero_calls / $total_calls * 100, 2) : 0;
                    $asr_ = intval($busy_calls) + intval($cancel_calls) + intval($not_zero_calls);
                    $ready_data3[$key]['asr'] = !empty($asr_) ? round($not_zero_calls / $asr_ * 100, 2) : 0;
...
//                    }
                    ...
                    foreach ($ready_data3 as $item_results_item) {
                        $time_key = $item['stat_time'];
                        if (array_key_exists($time_key,$draw_data[$show_acd])){
                            $call_data3[$show_acd][$time_key] = array($time_key, $draw_data[$show_acd][$time_key][1] + (float)$item_results_item[$show_acd]);
                            $call_data3[$show_abr][$time_key] = array($time_key, $draw_data[$show_abr][$time_key][1] + (float)$item_results_item[$show_abr]);
                            $call_data3[$show_asr][$time_key] = array($time_key, $draw_data[$show_asr][$time_key][1] + (float)$item_results_item[$show_asr]);
                            $call_data3[$show_pdd][$time_key] = array($time_key, $draw_data[$show_pdd][$time_key][1] + (float)$item_results_item[$show_pdd]);
                        }else {
                            $call_data3[$show_acd][$time_key] = array($time_key, (float)$item_results_item[$show_acd]);
                            $call_data3[$show_abr][$time_key] = array($time_key, (float)$item_results_item[$show_abr]);
                            $call_data3[$show_asr][$time_key] = array($time_key, (float)$item_results_item[$show_asr]);
                            $call_data3[$show_pdd][$time_key] = array($time_key, (float)$item_results_item[$show_pdd]);
                        }
                    }
                    sort($call_data3[$show_acd]);
                    sort($call_data3[$show_abr]);
                    sort($call_data3[$show_asr]);
                    sort($call_data3[$show_pdd]);

                    array_push($draw_data[$show_acd], array(
                        'name' => $item[0]['name'],
                        'data' => $call_data3[$show_acd],
                        'tooltip' => array(
                            'valueDecimals' => 2,
                        )
                    ));
...
//                    }
                    ...
                    foreach ($ready_data3 as $item_results_item) {
                        $time_key = $item['stat_time'];
                        if (array_key_exists($time_key,$draw_data[$show_acd])){
                            $call_data3[$show_revenue][$time_key] = array($time_key, $draw_data[$show_revenue][$time_key][1] + (float)$item_results_item[$show_revenue]);
                            $call_data3[$show_profitability][$time_key] = array($time_key, $draw_data[$show_profitability][$time_key][1] + (float)$item_results_item[$show_profitability]);
                        }else {
                            $call_data3[$show_revenue][$time_key] = array($time_key, (float)$item_results_item[$show_revenue]);
                            $call_data3[$show_profitability][$time_key] = array($time_key, (float)$item_results_item[$show_profitability]);
                        }
                    }
                    sort($call_data3[$show_revenue]);
                    sort($call_data3[$show_profitability]);
                    array_push($draw_data[$show_revenue], array(
                        'name' => $item[0]['name'],
                        'data' => $call_data3[$show_revenue],
                        'tooltip' => array(
                            'valueDecimals' => 2,
                        )
                    ));
                    array_push($draw_data[$show_profitability], array(
                        'name' => $item[0]['name'],
                        'data' => $call_data3[$show_profitability],
                        'tooltip' => array(
                            'valueDecimals' => 2,
                        )
                    ));
...
// end channel
    ...
    public function get_draw_trunk_data($type, $duration, $trunk, $trunk_ip, $server)
    {
        $time = $this->get_time_by_duration($duration);
        $draw_data = array(
            'call' => array(
        ...
        if (empty($trunk_ip))
        {
            $sql_total = "SELECT to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(call) as call, sum(cps) as cps, sum(channels) as channel FROM qos_resource WHERE
...
ORDER BY report_time ASC";
            if ($duration >= 7)
                $sql_total = "SELECT to_char(report_time, 'YYYY-MM-DD HH24:00:00') as stat_time, max(call) as call, max(cps) as cps, max(channel) as channel FROM (SELECT report_time, sum(call) as call, sum(cps) as cps, sum(channels) as channel FROM qos_resource WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition
GROUP BY report_time) AS t GROUP BY report_time
ORDER BY 1 ASC";
        } else
        {
            $sql_total = "SELECT to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(call) as call, sum(cps) as cps, sum(channels) as channel FROM qos_ip WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition
GROUP BY report_time
ORDER BY report_time ASC";
            if ($duration >= 7)
                $sql_total = "SELECT to_char(report_time, 'YYYY-MM-DD HH24:00:00') as stat_time, max(call) as call, max(cps) as cps, max(channel) as channel FROM (SELECT report_time, sum(call) as call, sum(cps) as cps, sum(channels) as channel FROM qos_ip WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP AND direction = {$type} $server_condition
GROUP BY report_time) AS t GROUP BY report_time
ORDER BY 1 ASC";
        }
...
//            array_push($draw_data['call'][0]['data'], array(strtotime($total_item[0]['stat_time']) * 1000, (float) $total_item[0]['call']));
////            if ($total_item[0]['cps'] > 0)
//            array_push($draw_data['cps'][0]['data'], array(strtotime($total_item[0]['stat_time']) * 1000, (float) $total_item[0]['cps']));
////            if ($total_item[0]['channel'] > 0)
//            array_push($draw_data['channel'][0]['data'], array(strtotime($total_item[0]['stat_time']) * 1000, (float) $total_item[0]['channel']));
//        }

        foreach ($total_results as $total_item) {
//if ($total_item[0]['call'] > 0)
            $time_key = strtotime($total_item[0]['stat_time']) * 1000;
            if (array_key_exists($time_key,$draw_data['call'][0]['data'])){

                $draw_data['call'][0]['data'][$time_key] = array($time_key,$draw_data['call'][0]['data'][$time_key][1]+(int)$total_item[0]['call']);
                $draw_data['cps'][0]['data'][$time_key] = array($time_key,$draw_data['cps'][0]['data'][$time_key][1]+(int)$total_item[0]['cps']);
                $draw_data['channel'][0]['data'][$time_key] = array($time_key,$draw_data['call'][0]['data'][$time_key][1]+(int)$total_item[0]['channel']);
            } else {

                $draw_data['call'][0]['data'][$time_key] = array(strtotime($total_item[0]['stat_time']) * 1000, (int)$total_item[0]['call']);

                $draw_data['cps'][0]['data'][$time_key] = array(strtotime($total_item[0]['stat_time']) * 1000, (int)$total_item[0]['cps']);

                $draw_data['channel'][0]['data'][$time_key] = array(strtotime($total_item[0]['stat_time']) * 1000, (int)$total_item[0]['channel']);

            }
        }
        sort($draw_data['call'][0]['data']);
        sort($draw_data['cps'][0]['data']);
        sort($draw_data['channel'][0]['data']);
...
// ORIG
        ...
        if (!empty($trunk_ip))
        {
            ...
            foreach ($result as $item)
            {
                ...
                $sql_item = "SELECT
to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(call) as call, sum(cps) as cps, sum(channels) as channel
FROM qos_ip
WHERE
...
ORDER BY report_time ASC";
                if ($duration >= 7)
                {
                    $sql_item = "SELECT to_char(report_time, 'YYYY-MM-DD HH24:00:00') as stat_time, max(call) as call, max(cps) as cps, max(channel) as channel FROM (SELECT
...
//                }
                ...
                foreach ($item_results as $item_item) {
                    $time_key = strtotime($item_item[0]['stat_time']) * 1000;
                    if (array_key_exists($time_key,$call_data)){

                        $call_data[$time_key] = array($time_key,$call_data[$time_key][1]+(int)$item_item[0]['call']);
                        $cps_data[$time_key] = array($time_key,$cps_data[$time_key][1]+(int)$item_item[0]['cps']);
                        $channel_data[$time_key] = array($time_key,$call_data[$time_key][1]+(int)$item_item[0]['channel']);
                    } else {

                        $call_data[$time_key] = array(strtotime($item_item[0]['stat_time']) * 1000, (int)$item_item[0]['call']);

                        $cps_data[$time_key] = array(strtotime($item_item[0]['stat_time']) * 1000, (int)$item_item[0]['cps']);

                        $channel_data[$time_key] = array(strtotime($item_item[0]['stat_time']) * 1000, (int)$item_item[0]['channel']);
        ...
        else
        {
            if (!ctype_digit($trunk))
            {
                ...
                foreach ($result as $item)
                {
                    ...
                    $sql_item = "SELECT
to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(call) as call
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
...
//                    }
                ...
                foreach ($result as $item)
                {
                    ...
                    $sql_item = "SELECT
to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(cps) as cps
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
//                    }
                    ...
                    foreach ($item_results as $item_item) {
                        $time_key = strtotime($item_item[0]['stat_time']) * 1000;
                        if (array_key_exists($time_key,$cps_data)){
                            $cps_data[$time_key] = array($time_key,$cps_data[$time_key][1]+(int)$item_item[0]['call']);
                        } else {
                            $cps_data[$time_key] = array(strtotime($item_item[0]['stat_time']) * 1000, (int)$item_item[0]['call']);
                        }
                    }
                    sort($cps_data);
...
//beign channel
                ...
                foreach ($result as $item)
                {
                    ...
                    $sql_item = "SELECT
to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') as stat_time, sum(channels) as channel
FROM qos_resource
WHERE
report_time BETWEEN CURRENT_TIMESTAMP - interval '{$time}' AND CURRENT_TIMESTAMP
AND res_id = {$item[0]['res_id']} AND direction = {$type} $server_condition
...
//                    }
                    ...
                    foreach ($item_results as $item_item) {
                        $time_key = strtotime($item_item[0]['stat_time']) * 1000;
                        if (array_key_exists($time_key,$channel_data)){
                            $channel_data[$time_key] = array($time_key,$channel_data[$time_key][1]+(int)$item_item[0]['call']);
                        } else {
                            $channel_data[$time_key] = array(strtotime($item_item[0]['stat_time']) * 1000, (int)$item_item[0]['call']);
                        }
                    }
                    sort($channel_data);
...
//        echo "<pre>";  print_r($sum_e);
    ...
    {
        $search_time = date('Y-m-d H:i:s' ,strtotime($time) - 60);
        $search_end_time = date('Y-m-d H:i:s' ,strtotime($time) - 1);
        if ($server != NULL)
        {
            $server_condition = " AND server_ip = '{$server[0]}' AND server_port = $server[1]";
        }
...
//WHERE to_char(report_time - interval '1 mins','YYYY-MM-DD HH24:MI:00') = '$time' $server_condition";
        $sql = "SELECT sum(call) as call, sum(ingress_cps) as cps, sum(channels) as channel FROM qos_total
