...
class clients extends AppModel {
    ...
    function insertResource1($ingress, $egress, $alias, $client_id,$rate_table_id,$enough_balance) {
        $sql = "insert into resource (ingress, egress, alias, client_id,rate_table_id,enough_balance, media_type, t38) 
               values({$ingress}, {$egress}, '{$alias}', {$client_id}, {$rate_table_id},'{$enough_balance}', 2, true)  RETURNING resource_id";
        $res_id = $this->query($sql);
        return $res_id;
    }
    ...
    function insertResource2($ingress, $egress, $alias, $carrier) {
        $res_id = $this->query("insert into resource (ingress, egress, alias, client_id, media_type, t38) 
               values({$ingress}, {$egress}, '{$alias}', (select client_id from client where name = '{$carrier}'), 2, true)  RETURNING resource_id");
        return $res_id;
    }

    function insertResource3($ingress, $egress, $alias, $client_id,$enough_balance) {
        $res_id = $this->query("insert into resource (ingress, egress, alias, client_id,enough_balance, media_type, t38) 
               values({$ingress}, {$egress}, '{$alias}', {$client_id},'{$enough_balance}', 2, true)  RETURNING resource_id");
        return $res_id;
    }
    ...
    function insertResourcePrefix($id, $rate_table_id, $route_strategy_id, $tech_prefix, $product_id) {
        $this->query("insert into resource_prefix (resource_id, rate_table_id, route_strategy_id, tech_prefix, product_id)
            values({$id}, {$rate_table_id}, {$route_strategy_id}, '{$tech_prefix}', '{$product_id}')");
    }
...
//        return $this->query("INSERT INTO credit_application(client_id) VALUES ({$client_id})");
    ...
    public function get_auto_invoice_client($find_arr = array())
    {
        return $this->find('all',$find_arr);
    }
