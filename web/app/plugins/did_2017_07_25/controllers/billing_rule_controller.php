...
//        $tmpData = $this->DidBillingPlan->find("all");
//        die(var_dump($tmpData));
        $this->paginate = array(
            'fields' => array(
                'id', 'did_price', 'min_price', 'name', 'monthly_charge', 'rate_table_id', 'payphone_subcharge'
            ),
            'limit' => 100,
            'order' => array(
                'id' => 'desc',
        ...
        foreach ($this->data as &$item) {
            if(!empty($item['DidBillingPlan']['rate_table_id'])) {
                ...
                $item['DidBillingPlan']['rate_type'] = $tmpData['Rate']['rate_type'] == 0 ? 'Fixed Rate' : 'Variable Rate';
            } else {
                $item['DidBillingPlan']['rate_type'] = 'Fixed Rate';
            }
        }
    }
