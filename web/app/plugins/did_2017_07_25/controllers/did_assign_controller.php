...
class DidAssignController extends DidAppController
{
    ...
    public function change_status($number, $status)
    {
        ...
        if ($status == 0)
        {
            $this->Session->write('m', $this->DidAssign->create_json(201, __('The  status of  number  [%s] is inactived successfully!', true, $number)));
        }
        else
        {
            $this->Session->write('m', $this->DidAssign->create_json(201, __('The  status of  number  [%s] is actived successfully!', true, $number)));
        }
    ...
    public function action_edit_panel($egress_id = '', $number = null)
    {
        ...
        if ($this->isPost())
        {
            if ($number != null)
            {
                ...
                $this->Session->write('m', $this->DidAssign->create_json(201, __('The number of [%s] is modified successfully!', true, $this->data['DidAssign']['number'])));
            ...
            {
                $this->Session->write('m', $this->DidAssign->create_json(201, __('The number of [%s] is created successfully!', true, $this->data['DidAssign']['number'])));
                $log_flg = TRUE;
                $action = 0;
