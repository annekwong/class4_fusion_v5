                    ...
                    <table id="repository" class="list footable table table-striped tableTools table-bordered  table-white table-primary" style="overflow-x: hidden;display: none;">
                        <thead>
                          <tr>
                              ...
                              <th rowspan="2"><?php echo $appCommon->show_order('code', 'DID'); ?></th>
                              <th rowspan="2"><?php echo $appCommon->show_order('vendor_name', 'DID Vendor'); ?></th>
                              <th rowspan="2"><?php echo $appCommon->show_order('vendor_rule', 'Vendor Billing Rule'); ?></th>
                              <th rowspan="2"><?php echo $appCommon->show_order('client_name', 'DID Client'); ?></th>
                              <th rowspan="2"><?php echo $appCommon->show_order('client_rule', 'Client Billing Rule'); ?></th>
                <!--                                <th colspan="4">--><?php //__('ANI'); ?><!--</th>-->
                <!--                                <th colspan="4">--><?php //__('DNIS'); ?><!--</th>-->
                              <th rowspan="2"><?php __('Created Time') ?></th>
...
<!--                                <th colspan="4">--><?php //__('DNIS'); ?><!--</th>-->
                            ...
                            foreach ($data as $item)
                            {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo isset($item[0]['vendor_name']) ? $item[0]['vendor_name']: (isset($ingresses[$item[0]['vendor_id']]) ? $ingresses[$item[0]['vendor_id']] : '[Vendor has been deleted]'); ?></td>
                                    ...
                                    <td>
                                        ...
                                        <a title="<?php __('Delete') ?>" onclick="return myconfirm('Are you sure to delete the number[<?php echo $item[0]['code'] ?>] ?', this);" class="delete" href='<?php echo $this->webroot; ?>did/did/delete_did/<?php echo base64_encode($item[0]['id']); ?>/<?php echo isset($this->params['pass'][0]) ? $this->params['pass'][0] : ''; ?>'>
...
//                    }
        ...
        jQuery('a.edit_item').click(function() {
            ...
            jQuery(this).parent().parent().trAdd({
                ...
                callback:function(){
                    let trAdd = $('.add_edit');
                    let ani_dnis = $('#ani_dnis').val();
                    if(ani_dnis){
                        let group = $('#ani_dnis').attr('data-group');
                        let action = $('#ani_dnis').attr('data-action');
                        $('select.actions[data-group="' +group+ '"]').val(action);
                        $('[data-group="'+group+'"][data-action="'+action+'"]').val(ani_dnis);
                    }
                    trAdd.find('select.actions').on('change', function(){
                        if($(this).val()){
                            trAdd.find('select.actions[data-group!="'+$(this).attr("data-group")+'"]').val('');
