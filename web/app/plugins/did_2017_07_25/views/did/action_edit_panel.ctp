    ...
    <table>
        <tr class="add_edit">
            ...
            <td><?php
                ...
                {
                    echo $this->data['code'];
                    echo $xform->input('code', array('maxlength' => 20, 'type' => "hidden",'class' => 'did_number'));
                }
                else
                {
                    echo $xform->input('code', array('maxlength' => 20, 'type' => "text",'class' => 'did_number validate[required,custom[onlyNumber]]'));
                }
                ?>
            </td>
            ...
            <td>
                <select style="width: 150px;" name="data[vendorBillingId]" class="validate[required]">
                    ...
                    ?>
                        <option value="<?php echo $key; ?>" <?php echo (isset($this->data['vendorbillingid'] ) &&  $this->data['vendorbillingid'] == $key )? "selected" : "" ;?>><?php echo $billingRule; ?></option>
                    <?php
                    }
                    ?>
                </select>
            ...
            <td>
                <select style="width: 150px;" name="data[clientBillingId]" class="validate[required]">
                    ...
                    foreach ($billingRules as $key => $billingRule) {
                        ?>
                        <option value="<?php echo $key; ?>" <?php echo (isset($this->data['clientbillingid'] ) &&  $this->data['clientbillingid'] == $key )? "selected" : "" ;?>><?php echo $billingRule; ?></option>
                        <?php
                    }
                    ?>
                </select>
                <input type="hidden" id="ani_dnis" data-action= "<?php echo isset($this->data['action']) ? $this->data['action'] : '';?>" data-group= "<?php echo isset($this->data['group']) ? $this->data['group'] : '';?>" value="<?php echo isset($this->data['ani_dnis']) ? $this->data['ani_dnis'] : '';?>">
            </td>
            <td></td>
            <td></td>
            <td></td>
