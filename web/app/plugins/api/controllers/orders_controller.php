...
class OrdersController extends ApiAppController
{
    ...
    {        
        ...
        if ($this->RequestHandler->ispost())
        {
            ...
            $cart = new ShoppingCart();
            $this->Session->write('shopping_cart', serialize($cart));
            $this->Session->write('m', $this->DidAssign->create_json(201, __('Your Request %s has been submitted', true, $request_id)));
    ...
    public function trunk_panel($id = null, $trunk_type = 'egress')
    {
        ...
        if ($this->isPost()) {
            ...
            if(!empty($id))
            {
                ...
                if($ip)
                {
                    $sql .="update resource_ip set ip = network('{$ip}') where resource_id = {$id};";
                }
                if($prefix)
                {
