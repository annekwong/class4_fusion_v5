...
class DidRepos extends DidAppModel
{
    ...
    public function get_group_area_code()
    {
        ...
        */
        ...
        $sql = "select substring(number from 1 for 4)  as name,count(*) as count from ingress_did_repository where status = 1 and country = 'US' group by substring(number from 1 for 4) order by name";
        $result = $this->query($sql);
        return $result;   
    }
...
where substring(number from 1 for 4) = '{$area_code}' and status = 1) as t";
        $count_result = $this->query($count_sql);
