...
class DidRepossController extends DidAppController
{
    ...
    var $name = 'DidReposs';
    var $uses = array('did.DidRepos', 'ImportExportLog', 'did.DidAssign', "prresource.Gatewaygroup", 'did.DidBillingPlan', 'did.OrigLog', "Systemparam");
    ...
    public function action_edit_panel($ingress_id = '', $number = null)
    {
        ...
        if ($this->isPost())
        {
            if ($number != null)
            {
                ...
                $this->Session->write('m', $this->DidRepos->create_json(201, __('The number of [%s] is modified successfully!', true, $this->data['DidRepos']['number'])));
            ...
            else
            {
                ...
                $this->Session->write('m', $this->DidRepos->create_json(201, __('The number of [%s] is created successfully!', true, $this->data['DidRepos']['number'])));
                $log_flg = TRUE;
                $action = 0;
...
//            $this->data['DidRepos']['state'] = 'AZ';
    ...
    public function change_status($number, $status, $ingress_id = '')
    {
        ...
        if ($status == 0)
        {
            $this->Session->write('m', $this->DidRepos->create_json(201, __('The  status of  number  [%s] is inactived successfully!', true, $number)));
        }
        else
        {
            $this->Session->write('m', $this->DidRepos->create_json(201, __('The  status of  number  [%s] is actived successfully!', true, $number)));
        }
        ...
        $this->DidRepos->query($sql);
        $this->Session->write('m', $this->DidRepos->create_json(201, __('The  status of  number  [%s] is changed successfully!', true, $number)));
        $this->xredirect("/did/did_reposs/index/" . $ingress_id);
    }
    ...
    public function delete($number, $ingress_id = '')
    {
        ...
        $this->Session->write('m', $this->DidRepos->create_json(201, __('The  number  [%s] is deleted successfully!', true, $number)));
        $this->xredirect("/did/did_reposs/index/" . $ingress_id);
    }
