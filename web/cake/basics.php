 ...
 */
	function __($singular, $return = false, $replace_arr = array()) {
		if (!$singular) {
			return;
		}
        ...
        $tran = I18n::translate($singular);
        if(!empty($replace_arr)){
            if(is_array($replace_arr)){
                $search_arr = array_fill(0, count($replace_arr),"'%s'");
                $replace_arr = @array_map(function(&$i){return strval($i);},$replace_arr);
                $tran = preg_replace($search_arr,$replace_arr,$tran,1);
            } elseif(is_string($replace_arr)) {
                $search_arr = "'%s'";
                $tran = preg_replace($search_arr,$replace_arr,$tran);
            }
        }
 ...
 */
    ...
    function strip_whitespace($content) {
        $stripStr   = '';
        //分析php源码
        $tokens     = token_get_all($content);pr($tokens);exit;
        $last_space = false;
